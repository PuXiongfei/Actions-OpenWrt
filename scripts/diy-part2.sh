...
sed -i '/DEFAULT_PACKAGES.router/a\\tautomount autosamba ddns-scripts_cloudflare.com-v4 ipref3 ipv6helper luci-app-argon-config luci-app-easymesh luci-app-ttyd luci-app-webadmin wpad \\' include/target.mk
sed -n '/DEFAULT_PACKAGES.router/,/^ifneq/p' include/target.mk
...
if [ "$CONFIG_FILE_DEVICE" = "D2" ]; then
    ...
    sed -i '/d-team_newifi-d2$/,/d-team_newifi-d2$/{s/\\/luci-app-adguardhome luci-app-aria2 luci-app-mwan3helper luci-app-openclash luci-app-passwall luci-app-wireguard luci-app-zerotier \\/}' target/linux/ramips/image/mt7621.mk
    sed -n '/d-team_newifi-d2$/,/d-team_newifi-d2$/p' target/linux/ramips/image/mt7621.mk
    echo "修改passwall默认值"
...
if [ "$CONFIG_FILE_DEVICE" = "K3" ]; then
    ...
    sed -i '/phicomm_k3$/,/phicomm_k3$/{/DEVICE_PACKAGES/s/$/& autocore-arm luci-app-adguardhome luci-app-aria2 luci-app-mwan3helper luci-app-netdata luci-app-openclash luci-app-passwall luci-app-rclone luci-app-wireguard luci-app-zerotier/}' target/linux/bcm53xx/image/Makefile
    sed -n '/phicomm_k3$/,/phicomm_k3$/p' target/linux/bcm53xx/image/Makefile
...
if [ "$CONFIG_FILE_DEVICE" = "R3G" ]; then
    ...
    sed -i '/xiaomi_mi-router-3g$/,/xiaomi_mi-router-3g$/{s/\\/luci-app-adguardhome luci-app-aria2 luci-app-mwan3helper luci-app-netdata luci-app-openclash luci-app-passwall luci-app-rclone luci-app-wireguard luci-app-zerotier \\/}' target/linux/ramips/image/mt7621.mk
    sed -n '/xiaomi_mi-router-3g$/,/xiaomi_mi-router-3g$/p' target/linux/ramips/image/mt7621.mk
    echo "修改passwall默认值"
    sed -n '/INCLUDE_Shadowsocks_Libev_Client$/,/default/p' package/custom/openwrt-passwall/luci-app-passwall/Makefile
    sed -i '/INCLUDE_Shadowsocks_Libev_Client$/,/default/{s/default y/default n/}' package/custom/openwrt-passwall/luci-app-passwall/Makefile
...
if [ "$CONFIG_FILE_DEVICE" = "R86S" ]; then
    ...
    sed -i '/DEFAULT_PACKAGES/a\luci-app-adguardhome luci-app-aria2 luci-app-mwan3helper luci-app-docker luci-app-netdata luci-app-openclash luci-app-passwall luci-app-rclone \\' target/linux/x86/Makefile
    sed -n '/DEFAULT_PACKAGES/,/BuildTarget/p' target/linux/x86/Makefile
